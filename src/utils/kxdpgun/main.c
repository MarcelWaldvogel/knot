...
static void clear_stats(kxdpgun_stats_t *st)
{
	...
	st->wire_recv   = 0;
	st->collected   = 0;
	memset(st->rcodes_recv, 0, sizeof(st->rcodes_recv) * sizeof(*st->rcodes_recv));
	pthread_mutex_unlock(&st->mutex);
}

static size_t collect_stats(kxdpgun_stats_t *into, const kxdpgun_stats_t *what) // local stats (= what) have no mutex
{
				...
				if (ctx->tcp) {
					...
					for (int irl = 0; irl < n_relays; irl++) {
						...
						case XDP_TCP_ESTABLISH:
							...
							break;
						case XDP_TCP_DATA:
							if (check_dns_payload(&rl->data, ctx, &local_stats.ans_recv, &local_stats.size_recv)) {
					...
					for (int i = 0; i < recvd; i++) {
						(void)check_dns_payload(&pkts[i].payload, ctx, &local_stats.ans_recv, &local_stats.size_recv);
					}
				}
